...
/**
 * XML Pull Parser is an interface that defines parsing functionlity provided in <a
 * href="http://www.xmlpull.org/">XMLPULL V1 API</a> (visit this website to learn more about API and its
 * implementations).
 * <p/>
 * <p>
 ...
 * that <em>no DOCDECL is present</em> in XML documents (internal entites can still be defined with
 * defineEntityReplacementText()). This mode of operation is intened <b>for operation in constrained environments</b>
 * such as J2ME.
 * </ul>
 * <p/>
 * <p/>
 ...
 * <dd>Text content was read; the text content can be retreived using the getText() method. (when in validating mode
 ...
 * href="http://xmlpull.org/v1/doc/properties.html#xmldecl-version">http://xmlpull
 * .org/v1/doc/properties.html#xmldecl-version</a>&quot;) returns String ("1.0") or null if XMLDecl was not read or if
 * property is not supported
 * <li><b>standalone</b>: getProperty(&quot;<a
 * href="http://xmlpull.org/v1/doc/features.html#xmldecl-standalone">http://xmlpull
 * .org/v1/doc/features.html#xmldecl-standalone</a>&quot;) returns Boolean: null if there was no standalone declaration
 * or if property is not supported otherwise returns Boolean(true) if standalon="yes" and Boolean(false) when
 * standalone="no"
 ...
 * <p/>
 * <p>
 * For more details on API usage, please refer to the quick Introduction available at <a
 * href="http://www.xmlpull.org">http://www.xmlpull.org</a>
 * 
...
public interface XmlPullParser {
     ...
     * skipping IGNORABLE_WHITESPACE, PROCESSING_INSTRUCTION and COMMENT events, In contrast, <a
     * href="#nextToken()">nextToken()</a> will stop reading text when any other event is observed. Also, when the state
     * was reached by calling next(), the text value will be normalized, whereas getText() will return unnormalized
     * content in the case of nextToken(). This allows an exact roundtrip without chnanging line ends when examining low
     * level events, whereas for high level applications the text is normalized apropriately.
     * 
     * @see #next
     * @see #nextToken
     * @see #getText
    ...
    /**
     * A CDATA sections was just read; this token is available only from calls to <a
     * href="#nextToken()">nextToken()</a>. A call to next() will accumulate various text events into a single event of
     * type TEXT. The text contained in the CDATA section is available by callling getText().
     * 
     * @see #nextToken
     * @see #getText
     */
     ...
     * getTextt(); otherwise, the user is responsibile for resolving the entity reference. This event type is never
    ...
    /**
     * An XML processing instruction declaration was just read. This event type is available only via <a
     * href="#nextToken()">nextToken()</a>. getText() will return text that is inside the processing instruction. Calls
     * to next() will skip processing instructions automatically.
     * 
     * @see #nextToken
     * @see #getText
     */
    ...
    /**
     * An XML comment was just read. This event type is this token is available via <a
     * href="#nextToken()">nextToken()</a> only; calls to next() will skip comments automatically. The content of the
    ...
    /**
     * An XML document type declaration was just read. This token is available from <a
     * href="#nextToken()">nextToken()</a> only. The unparsed text inside the doctype is available via the getText()
     * method.
     * 
     * @see #nextToken
     * @see #getText
     ...
     * reported by nextToken() and ignored by next().
     * <p/>
     ...
     * all feautures are deactivated by default.
     * 
     ...
     * detection is supported then following feature <a
     * href="http://xmlpull.org/v1/doc/features.html#detect-encoding">http
     * ://xmlpull.org/v1/doc/features.html#detect-encoding</a> MUST be true amd otherwise it must be false
     * 
     * @param inputStream
     ...
     * null and the parser suppports the encoding detection feature
     ...
     * href="http://www.w3.org/TR/REC-xml#intern-replacement">XML 1.0 Section 4.5 Construction of Internal Entity
     ...
     * in DTD that has all special characters escaped: left angle bracket is replaced with &amp;lt;, ampersnad with
     * &amp;amp; and so on.
     * <p/>
     * <p>
    ...
    /**
     * Returns the namespace prefixe for the given position in the namespace stack. Default namespace declaration
     ...
     * <p/>
     * <p>
     * <strong>Please note:</strong> parser implementations may provide more efifcient lookup, e.g. using a Hashtable.
     * The 'xml' prefix is bound to "http://www.w3.org/XML/1998/namespace", as defined in the <a
     * href="http://www.w3.org/TR/REC-xml-names/#ns-using">Namespaces in XML</a> specification. Analogous, the 'xmlns'
     ...
     * example for TEXT event it is element content (this is typical case when next() is used).
     * <p/>
     ...
     * @return attribute namespace, empty string ("") is returned if namesapces processing is not enabled or namespaces
     ...
     * exception mus be thrown if entity reerence can not be exapnded). If element content is empty (content is "") then
     * no TEXT event will be reported.
     * <p/>
     * <p>
     ...
     * document round trip ie. reproduce exectly on output the XML input using getText(): returned content is always
     * unnormalized (exactly as in input). Otherwise returned content is end-of-line normalized as described <a
     * href="http://www.w3.org/TR/REC-xml#sec-line-ends">XML 1.0 End-of-Line Handling</a> and. Also when this feature is
     * enabled exact content of START_TAG, END_TAG, DOCDECL and PROCESSING_INSTRUCTION is available.
     * <p/>
     * <p>
     ...
     * all whitespace content outside root element will always reported as IGNORABLE_WHITESPACE otherise rteporting is
     * optional. <br>
     * Note: that element content may be delevered in multiple consecutive IGNORABLE_WHITESPACE events.
     * <dt>CDSECT
     * <dd>
     * return text <em>inside</em> CDATA (ex. 'fo&lt;o' from &lt;!CDATA[fo&lt;o]]>)
     ...
     * <dt>DOCDECL
     * <dd>
     * if FEATURE_XML_ROUNDTRIP is true or PROCESS_DOCDECL is false then return what is inside of DOCDECL for example it
     * returns:
     * 
     * <pre>
     ...
     * <p/>
     * <p>
     * <strong>NOTE:</strong> there is no gurantee that there will only one TEXT or IGNORABLE_WHITESPACE event from
