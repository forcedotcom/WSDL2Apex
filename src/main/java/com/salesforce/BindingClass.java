...
/**
 * This is a base class for generating binding aka client stubs in apex.
 * There are concrete versions of this that generate a traditional synchronous stub and a version that generates an async/continuations based stub.
 */
abstract class BindingClass extends AClass {
     ...
     protected void loadStub(APackage pkg, Binding binding, CertOptions certOptions, OutputHttpHeadersOptions httpOptions) throws CalloutException, ConnectionException {
         String loc = getLocation(definitions);
         AField endpoint = new AField("public", "String", CalloutConstants.ENDPOINT, "'" + loc + "'");
         fields.add(endpoint);

         AField inputHttpHeaders = new AField("public", "Map<String,String>",
                                              CalloutConstants.INPUT_HTTP_HEADERS, null);
         fields.add(inputHttpHeaders);

         if (httpOptions == OutputHttpHeadersOptions.IncludeHeaderMap) {
             AField outputHttpHeaders = new AField("public", "Map<String,String>", CalloutConstants.OUTPUT_HTTP_HEADERS, null);
         ...
         fields.add(new AField("private", "String[]",
                 "ns_map" + CalloutConstants.TYPE_INFO_PREFIX, nsMap()));
     }

     protected abstract void addOperation(APackage pkg, Binding binding, Operation operation) throws ConnectionException, CalloutException;
     ...
     private void addHeaders(Binding binding) throws ConnectionException, CalloutException {
         ...
         while(it.hasNext()) {
             ...
             String headerNS = "'" + part.getElement().getLocalPart() + "=" +
                     part.getElement().getNamespaceURI() + "'";
     ...
     private String getLocation(Definitions definitions) throws CalloutException {
         ...
         if (service == null) throw new CalloutException("Unable to find wsdl:service");
         Port port = service.getPort();
         if (port == null) throw new CalloutException("Unable to find wsdl:port");
         SoapAddress address = port.getSoapAddress();
         if (address == null) throw new CalloutException("Unable to find soap 1.1 address");
         String loc = address.getLocation();
         if (loc == null) throw new CalloutException("Unable to find endpoint location");
         return loc;
     }
